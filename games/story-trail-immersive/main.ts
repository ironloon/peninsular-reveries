...
// ── Colors ─────────────────────────────────────────────────────────────────
const SWATCHES = [0x1a1a2e, 0x2a2a4e, 0x88ccff, 0xffcc44, 0xffffff]
const C = {
  bg: SWATCHES[0],
  bgLight: SWATCHES[1],
  accent: SWATCHES[2],
...
async function boot(): Promise<void> {
  ...
  if (cameraGranted) {
    ...
    cameraPrompt.textContent = 'Camera access granted! Lean left or right to choose, raise hand to confirm.'
  } else {
  ...
  function gameLoop(_ticker: { deltaMS: number }): void {
    ...
    // Title text
    const titleText = new Text({ text: 'Story Trail Immersive', style: { fill: C.accent, fontSize: 24, fontFamily: 'system-ui', fontWeight: 'bold' } })
