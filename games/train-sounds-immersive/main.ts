...
// ── Colors ─────────────────────────────────────────────────────────────────
const SWATCHES = [0x1a2233, 0x2a3344, 0xff6644, 0x44ccff, 0xffd166]
const C = {
  bg: SWATCHES[0],
  bgLight: SWATCHES[1],
  accent: SWATCHES[2],
...
async function boot(): Promise<void> {
  ...
  if (cameraGranted) {
    ...
    cameraPrompt.textContent = 'Camera access granted! Point at different parts of the train on screen to trigger their sounds.'
  } else {
  ...
  function gameLoop(_ticker: { deltaMS: number }): void {
    ...
    // Title text
    const titleText = new Text({ text: 'Train Sounds Immersive', style: { fill: C.accent, fontSize: 24, fontFamily: 'system-ui', fontWeight: 'bold' } })
