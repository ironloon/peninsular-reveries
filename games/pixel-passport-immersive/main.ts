...
// ── Colors ─────────────────────────────────────────────────────────────────
const SWATCHES = [0x0a2040, 0x1a4060, 0xffd166, 0x44ccff, 0x88ff88]
const C = {
  bg: SWATCHES[0],
  bgLight: SWATCHES[1],
  accent: SWATCHES[2],
...
async function boot(): Promise<void> {
  ...
  if (cameraGranted) {
    ...
    cameraPrompt.textContent = 'Camera access granted! Point at destinations on the globe to select and explore them.'
  } else {
  ...
  function gameLoop(_ticker: { deltaMS: number }): void {
    ...
    // Title text
    const titleText = new Text({ text: 'Pixel Passport Immersive', style: { fill: C.accent, fontSize: 24, fontFamily: 'system-ui', fontWeight: 'bold' } })
