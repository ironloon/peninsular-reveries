...
// ── Colors ─────────────────────────────────────────────────────────────────
const SWATCHES = [0x0a0a2e, 0x1a1a4e, 0xff6600, 0x44aaff, 0xffffff]
const C = {
  bg: SWATCHES[0],
  bgLight: SWATCHES[1],
  accent: SWATCHES[2],
...
async function boot(): Promise<void> {
  ...
  function gameLoop(_ticker: { deltaMS: number }): void {
    ...
    // Title text
    const titleText = new Text({ text: 'Mission: Orbit Immersive', style: { fill: C.accent, fontSize: 24, fontFamily: 'system-ui', fontWeight: 'bold' } })
