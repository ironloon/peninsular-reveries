...
// ── Colors ─────────────────────────────────────────────────────────────────
const SWATCHES = [0x1a2428, 0x2a3438, 0x44ccaa, 0xffd166, 0xff8844]
const C = {
  bg: SWATCHES[0],
  bgLight: SWATCHES[1],
  accent: SWATCHES[2],
...
async function boot(): Promise<void> {
  ...
  if (cameraGranted) {
    ...
    cameraPrompt.textContent = 'Camera access granted! Point at items with your hand to pick them up, then point at spots to place them.'
  } else {
  ...
  function gameLoop(_ticker: { deltaMS: number }): void {
    ...
    // Title text
    const titleText = new Text({ text: 'Spot On Immersive', style: { fill: C.accent, fontSize: 24, fontFamily: 'system-ui', fontWeight: 'bold' } })
