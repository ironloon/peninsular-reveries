...
## Critique
Date: 2026-04-24
Reviewer: gnd-critique (interactive review with user)

### What Worked
- LEG-1 rename thorough and clean — zero residual references, all surfaces updated
- LEG-2 aria descriptions hit the accessibility bar (16 hotspots across 4 presets)
- LEG-3 departure/arrival animations and reduce-motion gating ship fully
- LEG-4 synth removal complete, gain raises verified at target values
- LEG-5/6 Spot On mechanically complete (pick-up/place/drop, rooms, completion, input)
- Attribution sync produces correct entries for both new games
- Build, unit tests, and delivery verification all pass

### What Didn't
- Spot On sounds entirely broken — `bundled: true` but no audio files on disk (LEG-6)
- Spot On room scene too narrow on iPhone portrait — 25%+ gap on each side (LEG-6, user observation)
- Two CSS classes added by JS but undefined in CSS: `.room-scene--complete`, `.room-spot--highlight` (LEG-6)
- Train Sounds hotspots invisible on touch — no hover on mobile, need persistent visual indicator (LEG-2, user observation)
- Rainbow overlay uses conic-gradient fan pattern, not a believable rainbow arc (LEG-3, user observation)
- Drop sound reuses pickup whoosh — same audio for "lift" and "let go" is confusing feedback (LEG-6)
- Drum pad is silent — `bundled: false` placeholders with no audio files (LEG-4, expected but regresses from prior working synth sounds)
- Audio audibility for train sounds unverified — diver skill's loudness probe was not run (LEG-4)

### Chart Gaps
- Spot On placement mechanic: "no wrong spot" was charted as "any spot accepts any item" but user wants freeform anywhere-on-surface placement, not fixed 1:1 spots
- Audio bundle-audit: workshop did not resolve whether `bundled: true` requires files on disk or is aspirational
- Sound ID count under-specified: 5 player actions but only 4 sample IDs enumerated

### User Effectiveness
- User intent was comprehensive — all gaps are chart-to-fidelity, not prompting gaps
- The Spot On mechanic mismatch is only visible on a real phone — no amount of plan review catches it

### Blockers
- None (build passes, tests pass, delivery is local-only)

### Corrections Applied
- `.pi/skills/gnd-critique/LOCAL.md` created: interactive hard-stop language; critique fixes process not product; write corrections to LOCAL.md files, not base SKILL.md/agent files
- `.pi/skills/gnd-chart/LOCAL.md` created: audio bundle-audit checkpoint; sound ID enumeration rule
- `.pi/agents/gnd-navigator-LOCAL.md` created: audio manifest review checkpoint; classList cross-reference extension
- `.planning/gnd-backlog.md` updated: removed completed "Music Pad rename" item, added Spot On + Train Sounds holding list items

### Community Candidates
- **gnd-critique base SKILL.md**: Add interactive review hard-stop language for PC Phase 2 (wait after each beat before advancing). Current skill says "Ask" but doesn't specify stopping.
- **gnd-critique base SKILL.md**: Add "don't edit SKILL.md, use LOCAL.md" guidance with explanation that npm updates will overwrite edits. All gnd skill/agent base files should carry this header.
- **gnd-critique base SKILL.md**: Add "critique fixes process, not product" rule — all implementation work goes through chart→navigator→diver, even minor bug fixes.
- **All gnd skill/agent base files**: Add LOCAL.md override pattern — "Do not edit this file directly. For project-local overrides, create or modify the associated LOCAL.md file."

### Field Review Holding List
(Transferred to `.planning/gnd-backlog.md` under Spot On and Train Sounds sections)